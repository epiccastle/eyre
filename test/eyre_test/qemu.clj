...
(defn stop [opts]
  ...
  (let [serial-socket (get opts :serial-socket "/tmp/freebsd-serial.sock")]
    (run! (format "echo 'system_powerdown' | socat - UNIX-CONNECT:%s"
                  (string/replace serial-socket #"-serial\.sock$" "-monitor.sock")))))
